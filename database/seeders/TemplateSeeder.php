...
class TemplateSeeder extends Seeder
{
    ...
    protected function createCatkinTemplate()
    {
        ...
        $table->addCell(2500)->addText('DASAR PELAKSANAAN', ['bold' => true]);
        $table->addCell(3500)->addText('URAIAN PEKERJAAN', ['bold' => true]);
        $table->addCell(2000)->addText('HASIL', ['bold' => true]);

        // Row for Cloning
        $table->addRow();
        $table->addCell(500)->addText('${no}');
        $table->addCell(2000)->addText('${hari_tanggal}');
        $table->addCell(2500)->addText('${dasar}');
        $table->addCell(3500)->addText('${uraian}');
        $table->addCell(2000)->addText('${output}');
